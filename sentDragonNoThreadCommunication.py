...
ser = serial.Serial("/dev/ttyUSB0", 115200, timeout=2)  # ms
...
def run(weights=ROOT / 'weights_self/block_weights/best.pt',
        device='0',
        imgsz=(640, 640),
        half=True,
        vid_stride=1,
...
3            """
            ...
            if target is not None:
                ...
                if ID is not None:
                    offset = target_order_dictionary.get(int(ID))  # offset 是一个list
                    x = len(offset)
                    if x == 0:
                        offset = 0
                    else:
                    ...
                    print("offset", offset)
                    serial_communication_output(offset*100)
        ...
        cv2.resizeWindow(str(p), 640, 480)  # im0.shape[1], im0.shape[0]
